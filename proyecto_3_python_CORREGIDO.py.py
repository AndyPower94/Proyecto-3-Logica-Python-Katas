...
def filtrar_animales (lista):                                                                                                                   # Definimos nuestra funcion
...
print(filtrar_animales(animales_prueba))
...
"""--12. Genera una función que al recibir una frase devuelva una lista con la longitud de cada palabra. Usa la función map()"""                      #CORREGIDO

def longitud_palabras(frase):                     # Definimos la función que recibe una frase
    palabras = frase.split()                             # Separamos la frase en palabras usando los espacios
    longitudes = list(map(len, palabras))    # Aplica len() a cada palabra usando map()
    return longitudes                                      # Devuelve la lista con las longitudes

print(longitud_palabras("Hola The Power School"))  
...
from functools import reduce                                          # Importamos reduce                                                                               #CORREGIDO

def lista_a_numero2(lista_digitos):                                 # Función que convierte una lista de dígitos en un número
    def combinar(acumulado, nuevo):                              # Función que define cómo se combinan los valores
        return acumulado * 10 + nuevo                               # Desplaza el número y añade el nuevo dígito
 
    resultado = reduce(combinar, lista_digitos, 0)           # Reduce con valor inicial 0
    return resultado                                                              # Devuelve el número final

print(lista_a_numero2([5, 7, 2])) 
...
"""--20. Para una lista con elementos tipo integer y string obtén una nueva lista sólo con los valores int. Usa la función filter()"""                                #CORREGIDO

lista = [10, "hola", 25, "mundo", 7]                             # Lista con enteros y strings

solo_enteros = list(
    filter(lambda x: isinstance(x, int), lista)                  # Filtra solo los valores de tipo int
)

print(solo_enteros)                                                        # Muestra únicamente los enteros
...
lanza una excepción."""                                                                                                                                                                                              #CORREGIDO

def buscar_nombre():                                                                                          # Creamos la función principal
    entrada = input("Ingresa nombres separados por coma: ")                     # Solicitamos los nombres
    lista = [nombre.strip() for nombre in entrada.split(",")]                            # Eliminamos espacios extra

    nombre_buscar = input("Ingresa el nombre a buscar: ").strip()               # Limpiamos también el nombre buscado

    if nombre_buscar in lista:                                                                                # Verificamos si el nombre existe
        print(f"El nombre {nombre_buscar} fue encontrado.")                         # Mensaje si se encuentra
    else:
        raise Exception("El nombre no está en la lista.")                                    # Lanzamos excepción si no existe


try:
    buscar_nombre()                                                                                              # Llamamos la función
except Exception as e:
    print(e)                                                                                                                # Mostramos el error

El nombre Carlos fue encontrado.
...
class Arbol:                                                                                # Definimos la clase Arbol
    def __init__(self):                                                                 # Constructor del árbol
        self.tronco = 1                                                                    # El tronco inicia con longitud 1
        self.ramas = []                                                                    # Lista vacía de ramas

    def crecer_tronco(self):                                                       # Aumenta el tronco
        self.tronco += 1                                                                  # Suma una unidad al tronco

    def nueva_rama(self):                                                          # Agrega una nueva rama
        self.ramas.append(1)                                                        # Cada rama comienza con longitud 1

    def crecer_ramas(self):                                                         # Hace crecer todas las ramas
        self.ramas = [rama + 1 for rama in self.ramas]              # Incrementa cada rama en 1

    def quitar_rama(self, posicion):                                           # Elimina una rama por índice
        if 0 <= posicion < len(self.ramas):                                     # Validamos que la posición exista
            self.ramas.pop(posicion)
        else:
            raise IndexError("Posición de rama inválida")           # Error si no existe
...
arbol.nueva_rama()                  # 3. Añadir rama
arbol.crecer_ramas()                # 4. Crecer ramas
arbol.nueva_rama()                  # 5. Añadir rama
arbol.nueva_rama()                  # 5. Añadir otra rama
arbol.quitar_rama(2)                # 6. Quitar rama en posición 2
print(arbol.info_arbol())           # 7. Mostrar información

Tronco: 2, Ramas: 2, Longitudes: [2, 1]
...
def contar_palabras(texto):  
    ...
    conteo = {}                                                                                                            # Diccionario para el conteo

    for p in palabras:                                                                                                # Recorremos cada palabra
        if p in conteo:
            conteo[p] += 1                                                                                            # Si ya existe, sumamos 1
        else:
            conteo[p] = 1                                                                                              # Si no existe, la iniciamos en 1

    return conteo                                                                                                      # Devuelve el diccionario


def reemplazar_palabras(texto, palabra_original, palabra_nueva):
    """Reemplaza una palabra por otra dentro del texto"""
    palabras = texto.split()                                                                                  # Separamos palabras
    resultado = []                                                                                                  # Lista para construir el nuevo texto

    for p in palabras:
        if p == palabra_original:
            resultado.append(palabra_nueva)                                                    # Reemplaza la palabra
        else:
            resultado.append(p)                                                                            # Mantiene la palabra original

    return " ".join(resultado)                                                                            # Reconstruye el texto


def eliminar_palabra(texto, palabra_eliminar):
    """Elimina una palabra del texto"""
    palabras = texto.split()                                                                                 # Separa palabras
    resultado = []                                                                                                 # Lista para el texto final

    for p in palabras:
        if p != palabra_eliminar:
            resultado.append(p)                                                                           # Agrega solo las palabras válidas

    return " ".join(resultado)                                                                           # Texto final sin la palabra eliminada


def procesar_texto(texto, opcion, *args):
    """
    Procesa el texto según la opción indicada:
    - 'contar'     → no recibe argumentos
    - 'reemplazar' → recibe palabra_original y palabra_nueva
    - 'eliminar'   → recibe palabra_a_eliminar
    """
    if opcion == "contar":
        if len(args) != 0:
            raise ValueError("La opción 'contar' no recibe argumentos.")
        return contar_palabras(texto)

    elif opcion == "reemplazar":
        if len(args) != 2:
            raise ValueError("La opción 'reemplazar' requiere 2 argumentos.")
        return reemplazar_palabras(texto, args[0], args[1])

    elif opcion == "eliminar":
        if len(args) != 1:
            raise ValueError("La opción 'eliminar' requiere 1 argumento.")
...
print("\nReemplazo de palabras:")
print(procesar_texto(texto_prueba, "reemplazar", "hola", "hi"))

print("\nEliminación de palabras:")
print(procesar_texto(texto_prueba, "eliminar", "mundo"))
...
"""- 38. Genera un programa que nos diga si es de noche, de día o tarde según la hora proporcionada por el usuario."""                            #CORREGIDO

try:
    hora = int(input("Introduce la hora (0-23): "))               # Pide la hora al usuario y la convierte a int

    if hora < 0 or hora > 23:                                                      # Valida que esté en el rango correcto
        print("Hora inválida")
    ...
    print("Error: Debes introducir un número entero.")     # Captura texto u otros valores no numéricos
...
try:
    calificacion = int(input("Introduce la calificación (0-100): "))  # Pide la nota y la convierte a int
    ...
    print("Error: Debes introducir un número entero.")                # Manejo de error por texto
